...
pub async fn get_conversion_rate(code1: &str, code2: &str){
    ...
    let api_call_str: String = format!("https://free.currconv.com/api/v7/convert?q={code1}_{code2}&compact=ultra&apiKey={api_key}");
    let result = reqwest::get(api_call_str).await.expect("REASON").text().await.unwrap();

    // Format the results
    let start_pos = result.find(":").unwrap() + 0; // Convert to int
    let end_pos = start_pos + 6;
    let final_results = &result[start_pos..end_pos];
    println!("Conversion Rate from {code1} to {code2}: {:?}", final_results);
}